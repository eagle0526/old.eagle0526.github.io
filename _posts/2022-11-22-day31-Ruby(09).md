...
**接下來會有以下幾個流程**  
一、點讚的icon放上去，並用JS寫出可以變換狀態的按鈕  
二、我們要用fetch去接住ruby傳來的狀態，因此先來做路徑(這個路徑是點讚後會觸發的路徑)
三、處理fetch，使用HTML傳Ruby值到JS(這個值就是網址上面的id，目前許願卡的id)
四、做一個多對多的model(因為要連結使用者、許願卡)
五、用Ruby傳到fetch的資料，判斷按鈕狀態(如果已經在約診簿裡，代表已按過讚)
六、設定剛進網站按鈕的狀態(如果已經按過讚，要是黑色的)，這個要用HTML來做雙向溝通
(Ruby傳是否已經在資料庫給HTML，HTML設定data，利用stimulus傳ruby給JS，最後用JS來做按鈕初始狀態)
七、整理、優化CODE

### 步驟一、點讚icon

#### fontawesome教學(可以增加icon)
...
#### 多對多關聯的命名 - 如果跟一堆多的關聯相撞如何解決
14:20 
不過遇到問題了，還記得前面我們有寫過，has_many :wish_lists的關聯嗎！？  
新創多對多的關聯跟他撞到了，那要怎麼解決呢？我們幫新的表改名就可以了  
...
```
我們這邊把has_many :users改掉的原因，是因為我們之後寫方法時，會跟user搞混，如果會搞混，那我們就改寫users的名稱，方便我們寫
搞混是什麼意思呢？？


**我來舉個例子**
首先我們來看wish_list的model有一些關聯
```md
