...
這樣就可以在新增留言的時候，刪除的按鈕就一起出來了，原因就是這一包檔案裡面的資料
在你新增留言時，都會一起渲染出來
...
上面這樣改還沒完喔！還記得我們的li區塊怎麼產生的嗎？就是用create.js產出並塞進去的
所以我們現在要去改create.js.erb的資料，把原本li的手打資料，改成渲染_comment檔案的方式
```md
> var comments = document.querySelector("#comments")
> var li = `<%= render "comment", comment: @comment %>`
> 
> comments.insertAdjacentHTML("afterbegin", li)
> document.querySelector("#comment_content").value = ""
```
