...
我們現在適用redirect_to來進行換頁，雖然有turbolinks的幫助，已經感覺不太到有換頁了，但是還是要等一下
因此我們今天想用一個更快的方式 -> 透過Ajax，當使用者留完言後，我們不要用重導的方式回網頁，我們用
Ajax的方式，把新增的留言透過JS，寫進留言區的欄位，這樣不就不用redirect_to了嗎！！！！
...
```md
> <%= form_with(model: @comment, url: wish_list_comments_path(@wish_list), local: false) do |f| %>
>   <div>
>       <div>留言：</div>
...
掛上去後，可以在show頁面加上一個刪除按鈕，刪除的路徑是comment的destroy，這時候可以幫link_to掛上使用XML送的方式(remote: true)
```md
> <% @comments.each do |comment| %>
> <li>
>   <%= comment.content %>
...
#### 刪除的render partial
！！但是這樣寫，今天新增留言的時候，刪除功能會不見，所以要使用render partial的寫法  

由於我們剛剛新增li的方法，是使用JS的insertAdjacentHTML，因此之後新增的留言，會吃不到刪除的功能
因此我們來修改一下

把剛剛整段的li，搬家搬到comment資料夾裡面，並在裡面新創一個_comment.html.erb的檔案
...
#### render collection(很重要)  
11/22 15:00  

不過這樣寫會以另外一個問題，效能很很爛，rails有說不建議再迴圈裡面使用render partial
...
#### 在create.js.erb寫render


原本寫法 var li 的寫法是寫一段留言的內容，並塞進留言欄位
之後我們來用render寫法試試
```md
> var comments = document.querySelector(".comments")
> 
...
？？？？？超級大疑問，這邊為啥要用render @comment來改寫JS，原本的寫法壞在哪裡
...
共同的東西都寫在_comment.html.erb這個資料夾，並且把他那一部分渲染過來，渲染過來的時後，裡面的comment變數是@comment
這樣寫的好處是，不管li寫的在複雜，我都可以把li透過render的方式渲染出來
```md
...
因為這樣寫就可以把所有的留言都抓出來，並且讓所有留言都有刪除功能  
這樣寫可以讓留言跑出來，就是因為我們是從資料庫抓出真正的資料，在塞到留言欄位
!!這樣刪除欄位就會一起生出來了!!
...
在data那邊加上remote: true，變成xml，這樣就可以用XML送出資料  

首先我們先假設，我們每一個刪除的id，他的id是comment_??，??是有規律的數字
...
接下來可以到destroy.js.erb，抓住id並刪掉
用@comment.id，來抓到指定要刪除了留言
```md
...
#### dom_id
不過這邊有個特別的寫法，叫做dom_id
這個helper會幫我們長出一段id出來

可以實際把它印出來看看，他會自動幫你長出id
...
不過JS放在同一個資料夾非常的醜，那要放到哪呢？？  
rails有一個放JS的地方 -> JavaScript檔案夾裡的packs -> application.js  
我們就把檔案搬過去  
  
不過搬過去後，發生了另外一個問題 -> script都先後渲染問題  
...
接下來要在rails上寫Stimulus.js  

這一段是官網的CODE，來解析一下
這樣寫，之後整段的生命週期都交由hello_controller管理
```md
...
#### 按鈕加上data-controller
把data-controller加在按鈕上，並新建一個資料夾叫做love_btn_controller.js
這邊加的controller，可以幫助把資料傳到JS那邊，並開始寫JS
```md
...
[turbo]: https://www.writershelf.com/article/rails-turbolinks%E2%84%A2-5-%E6%B7%B1%E5%BA%A6%E7%A0%94%E7%A9%B6?locale=zh-TW
